...
int sm2_sig_init(SM3_CTX* ctx, const SM2_PUB_KEY* key, uint8_t* id, int len){
	if(len*8>65535){
		return 1;
	}
...
int sm2_sig_update(SM3_CTX* ctx, uint8_t* message, size_t len){
...
void sm2_sig_generate(SM2_SIGNATURE* sig, SM2_PRI_KEY_EXT* key, uint8_t hash[32]){
	BN_256 e;
...
int sm2_sig_verify(SM2_SIGNATURE* sig, SM2_PUB_KEY* key, uint8_t hash[32]){
	if(bn_256_is_zero(sig->r)){
		return 1;
	}
